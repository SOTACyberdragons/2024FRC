...
public class RobotContainer {
    ...
    public RobotContainer() {
        ...
        //PP Auto Commands
        NamedCommands.registerCommand("ShootClose", new ShootClose(feeder,shooter));
        NamedCommands.registerCommand("ShooterOn", new ShooterOn(shooter));
        NamedCommands.registerCommand("Shooting", new Shooting(shooter));
        NamedCommands.registerCommand("FireCMD", new FireCMD(feeder,shooter));
        NamedCommands.registerCommand("SmartAutoIntake", new SmartAutoIntake(intake,feeder));
        NamedCommands.registerCommand("Bloop", new Bloop(feeder, shooter));
        NamedCommands.registerCommand("IntakeDown", new IntakeOut(intake));
        NamedCommands.registerCommand("NoteCheck", new NoteCheck(intake, feeder));
        NamedCommands.registerCommand("IntakeDown2F", new IntakeToFloor(intake));
    ...
    private void configureButtonBindings() {
        ...
        */
        ...
        //new POVButton(driver, 0).whileTrue(new FeederCMD(feeder,-1) );
        new POVButton(driver, 90).whileTrue(new IntakePivotCMD(intake,1));
        //new POVButton(driver, 270).whileTrue(new IntakeMotorCMD(intake,feeder,-1));
        new POVButton(driver, 0).whileTrue(new IntakeMotorCMD(intake,feeder,-1));
        new POVButton(driver, 180).whileTrue(new SmartIntakeTest(intake,feeder));

        //Shooting
        new Trigger(() -> driver.getRawAxis(rightTiggerAxis) > .5).whileTrue(new ShooterCMD(feeder,shooter) );
        new Trigger(() -> driver.getRawAxis(leftTiggerAxis) > .5).whileTrue(new ShootFireMan(feeder,shooter) );

        ampscore.whileTrue(new AmpScore(feeder, shooter));

        intakeSmartToggle.whileTrue(new SmartIntake(intake,feeder));
    ...
    private void configureSubsystemDefaults() {
        ...
        feeder.setDefaultCommand(new FeederDefaultCMD(feeder));
